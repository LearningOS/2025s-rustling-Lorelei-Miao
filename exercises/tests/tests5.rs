...
unsafe fn modify_by_address(address: usize) {
    // SAFETY: The address is assumed to be a valid pointer to a `u32` value.
    // We convert the address into a mutable pointer to `u32` and modify the value.
    let ptr: *mut u32 = address as *mut u32;
    if !ptr.is_null() {
        *ptr = 0xAABBCCDD; // Modify the value at the address to the specified value.
    }
}
